...
Retention <- RetentionIndicators %>%
  mutate(FiscalYear = year(ymd(entityYearsVector.i., truncated = 2)),
         OneYear = as.numeric(OneYear),
         ThreeYear = as.numeric(ThreeYear),
         FiveYear = as.numeric(FiveYear)) %>%
  select(!entityYearsVector.i.)
...
Majors_new <- Majors %>%
  ...
  pivot_wider(id_cols= Entity.ID, names_from = School.of.Graduation, values_from = Major, value_fn = list, unused_fn = max)
