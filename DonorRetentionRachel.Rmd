...
donorYr <- DonorHistory_Clean %>%
  group_by(Fiscal.Year)

# Stacked bar graph- credit amount by designation school colored by year
# More red = more recent donations
ggplot(donorYr, aes(fill=Fiscal.Year, y=Credit.Amount, x=Designation.School.Unit)) + 
  geom_bar(position="stack", stat="identity") +
...
#donorYr2 <- donorYr[which(donorYr$Fiscal.Year > 2010),]
nb.cols <- 22
mycolors <- colorRampPalette(brewer.pal(8, "Set1"))(nb.cols)

# Stacked bar graph- credit amount by designation school colored by year
# More red = more recent donations
ggplot(donorYr, aes(fill=factor(Fiscal.Year), y=Credit.Amount, x=Designation.School.Unit)) + 
  geom_bar(position="stack", stat="identity") +
